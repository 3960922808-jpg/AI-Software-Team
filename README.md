...
- 独立模型池：可添加、编辑、测试和删除多个加密模型连接
- 主 Agent 与 10 个专业子 Agent 可分别绑定不同模型，未绑定角色自动回退到主模型
...
- 独立无边框启动页：使用 SoftwareTeam 视觉稿，动态展示初始化阶段、进度与主窗口就绪切换
- 全新透明应用图标，覆盖工作台品牌位、窗口、安装程序与 Windows 快捷方式
- 白色玻璃界面与平滑角色动画，支持 Windows“减少动态效果”偏好
...
1. 工具执行：受控终端、代码编辑、测试与构建工具契约。
2. 记忆增强：向量检索、分段索引和可追溯引用。
3. 运行监控：任务耗时、令牌统计、告警与失败重试。
