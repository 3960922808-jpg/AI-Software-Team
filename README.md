...
- 可视化工作流：项目经理位于画布最左侧中部，直接分出 10 条任务支流连接 10 个专业 Agent；每个 Agent 再向右派发自己的产品、架构、开发、测试、审查或交付任务板块。

切换过程会先同步任务、记忆、技能与执行状态，并通过带进度条的加载动画完成界面切换。软件研发模板的 21 个节点和 20 条基础连线不是静态示意图，会读取当前任务的真实状态、子 Agent 运行记录、模型路由、已启用技能、自动检查和文件产物。

工作流画布支持软件研发、生图和视频三种模板。节点可以自由拖动并持久化位置，也可以添加、编辑或删除节点，自定义或删除连线，调整节点进度，并直接为经理或 Agent 选择模型。右键节点可打开编辑菜单；右侧 AI 对话框使用 `@` 指定经理或单个 Agent，使用 `/` 调用 Skill。技能中心可导入经过结构校验的自定义 Skill JSON，第三方脚本不会被执行。

`v0.17.0` 完善了工作流编辑体验：自定义连线带箭头、流动动画和鼠标预览，节点弹窗不再横向溢出，右键菜单会依据窗口边缘自动翻转。所有独立视图都有返回按钮，工作流模板在白色侧栏中保持清晰的黑白对比。
...
“模型与 API”页面内可以直接配置项目经理和十个专业 Agent：选择“主模型的子 Agent”时沿用主模型连接，也可以绑定模型池中的独立大模型。GitHub 令牌由 Windows 加密持久化保存；联网中心支持连接测试、私有仓库、SSH 地址、`.git` 地址和 GitHub 分支页面地址。
