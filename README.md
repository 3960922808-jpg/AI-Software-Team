...
- 可视化工作流：项目经理位于画布中央，直接分出 10 条任务支流连接 10 个专业 Agent；每个 Agent 再连接自己的产品、架构、开发、测试、审查或交付板块。

切换过程会先同步任务、记忆、技能与执行状态，并通过带进度条的加载动画完成界面切换。工作流的 22 个节点和 21 条连线不是静态示意图，会读取当前任务的真实状态、子 Agent 运行记录、模型路由、已启用技能、自动检查和文件产物。
