(function registerWorkflowState(root, factory) {
  ...
  const imageNodes = [
    ...
    { id: "image-positive", code: "想画", title: "想画什么", subtitle: "写清主体、场景、光线和画面风格", role: "产品经理 Agent", x: 520, y: 220, width: 380, height: 340, inputs: ["CLIP"], outputs: ["CONDITIONING"], parameters: { prompt: "" }, parameterSchema: [{ key: "prompt", label: "画面描述", type: "textarea", rows: 7, inline: true }] },
    { id: "image-negative", code: "不要", title: "不想出现什么", subtitle: "写下需要避开的瑕疵和多余元素", role: "测试 Agent", x: 520, y: 630, width: 380, height: 300, inputs: ["CLIP"], outputs: ["CONDITIONING"], parameters: { negativePrompt: "低质量，模糊，变形，错误文字" }, parameterSchema: [{ key: "negativePrompt", label: "需要排除的内容", type: "textarea", rows: 5, inline: true }] },
    { id: "image-reference", code: "参考图", title: "添加参考图片", subtitle: "可选，让生成结果参考人物、构图或风格", x: 520, y: 1060, width: 380, height: 300, inputs: [], outputs: ["IMAGE"], parameters: { referenceImagePath: "" }, parameterSchema: [{ key: "referenceImagePath", label: "参考图片", type: "image", inline: true }] },
    { id: "image-latent", code: "尺寸", title: "图片尺寸", subtitle: "设置宽度、高度和一次生成几张", x: 960, y: 1130, inputs: [], outputs: ["LATENT"], parameters: { width: 1024, height: 1024, batch: 1 }, parameterSchema: [{ key: "width", label: "宽度", type: "number", min: 256, max: 4096, step: 64 }, { key: "height", label: "高度", type: "number", min: 256, max: 4096, step: 64 }, { key: "batch", label: "生成数量", type: "number", min: 1, max: 4 }] },
    { id: "image-sampler", code: "质量", title: "生成质量", subtitle: "调节精细程度、描述贴合度和随机变化", role: "技术主管 Agent", x: 1080, y: 600, width: 350, height: 350, inputs: ["MODEL", "POSITIVE", "NEGATIVE", "LATENT", "IMAGE"], outputs: ["SAMPLES"], parameters: { seed: -1, steps: 28, cfg: 7, sampler: "euler", scheduler: "normal" }, parameterSchema: [{ key: "seed", label: "随机编号（-1 为每次随机）", type: "number" }, { key: "steps", label: "精细程度", type: "number", min: 1, max: 150 }, { key: "cfg", label: "描述贴合度", type: "number", min: 1, max: 30, step: 0.5 }, { key: "sampler", label: "绘制方式", type: "select", options: [{ value: "euler", label: "均衡" }, { value: "euler_a", label: "变化丰富" }, { value: "dpmpp_2m", label: "细节优先" }, { value: "ddim", label: "速度优先" }] }, { key: "scheduler", label: "细节安排", type: "select", options: [{ value: "normal", label: "默认" }, { value: "karras", label: "细节增强" }, { value: "exponential", label: "快速成形" }, { value: "sgm_uniform", label: "画面稳定" }] }] },
  ...
  const imageEdges = [
    ...
    { from: "image-negative", to: "image-sampler", sourcePort: "CONDITIONING", targetPort: "NEGATIVE" }, { from: "image-latent", to: "image-sampler", sourcePort: "LATENT", targetPort: "LATENT" }, { from: "image-reference", to: "image-sampler", sourcePort: "IMAGE", targetPort: "IMAGE" },
  ...
  const videoNodes = [
    ...
    { id: "video-prompt", code: "想拍", title: "想生成什么视频", subtitle: "描述人物动作、场景、镜头和画面风格", role: "产品经理 Agent", x: 500, y: 220, width: 390, height: 350, inputs: ["ENCODER"], outputs: ["CONDITIONING"], parameters: { prompt: "" }, parameterSchema: [{ key: "prompt", label: "视频内容描述", type: "textarea", rows: 7, inline: true }] },
    { id: "video-negative", code: "不要", title: "不想出现什么", subtitle: "写下需要避免的闪烁、变形和多余内容", x: 500, y: 650, width: 390, height: 290, inputs: ["ENCODER"], outputs: ["CONDITIONING"], parameters: { negativePrompt: "闪烁，抖动，变形，低质量" }, parameterSchema: [{ key: "negativePrompt", label: "需要排除的内容", type: "textarea", rows: 5, inline: true }] },
    { id: "video-reference", code: "参考图", title: "添加参考图片", subtitle: "可选，用一张图片作为视频开头或人物参考", x: 500, y: 1050, width: 390, height: 300, inputs: [], outputs: ["IMAGE"], parameters: { referenceImagePath: "" }, parameterSchema: [{ key: "referenceImagePath", label: "参考图片", type: "image", inline: true }] },
